// ═══════════════════════════════════════════════════════════════════════════════
// VIVO — instructor move / ungroup (CLASSROOM MODE).
//
// Instructor_Move_Ungroup_Shared_Spec_v3, Phase 3 rollout (vivo — first of the five,
// following winemaster). Consumes the EXISTING shared factory `makeMoveSeat` via the
// negotiation group-doc adapter; no shared game-server source is changed here — this file
// only builds vivo's OnlineContext and exports the one callable the panel needs.
//
 ...
 * far above any real negotiation group: a manual move never bounces on size. It is NOT a
 * real capacity, and because negotiation groups hold no bots, the bot-eviction path that
...
const onlineDef: OnlineDefinition = {
  ...
  // calls this (only topUpGroupWithBots does, which vivo does not export); it is present
  ...
  makeBotSeat: () => {
    throw new HttpsError('failed-precondition', 'Vivo is a negotiation game — bots are never used.')
  },
}
...
const ctx: OnlineContext = {
  def: vivoGameDef,
  online: onlineDef,
  adapter: makeNegotiationGroupAdapter(roleKeys(vivoGameDef.roles)),
}
 ...
 * that role's `<role>_participants` array (vivo's role keys: vivo, ads).
 */
